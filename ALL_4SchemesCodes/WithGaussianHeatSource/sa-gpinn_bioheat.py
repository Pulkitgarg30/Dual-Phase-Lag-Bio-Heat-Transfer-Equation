...
if __name__ == "__main__":
    ...
    np.save("sagpinn_bioheat_time.npy", np.array([total_time]))
    ...
    plt.title("SA-gPINN: theta(Fo) at x*")
    plt.legend(); plt.grid(True); plt.tight_layout()
    plt.savefig("sagpinn_bioheat_slice.png", dpi=150)
    plt.show()

    plt.figure(figsize=(6, 4))
    plt.pcolormesh(xs, Fos, Theta, shading='auto')
    plt.xlabel("x"); plt.ylabel("Fo")
    plt.title("SA-gPINN: theta(x, Fo)")
    plt.colorbar(); plt.tight_layout()
    plt.savefig("sagpinn_bioheat_heatmap.png", dpi=150)
    plt.show()

    # ----------------------
    # Save solution for comparison
    # ----------------------
    np.save("sagpinn_bioheat.npy", Theta)
    print("Saved: sagpinn_bioheat.npy")