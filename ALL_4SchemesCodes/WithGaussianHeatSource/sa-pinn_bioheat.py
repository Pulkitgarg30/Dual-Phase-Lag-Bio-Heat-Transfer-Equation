...
# ==============================================================
#  SA weights — 2 terms: PDE residual and BC
...
sa_vars = [log_lam_pde, log_lam_bc]
...
# ==============================================================
#  SA-PINN total weighted loss
#  lam_pde * loss_pde + lam_bc * loss_bc
#  SA weights trained adversarially
# ==============================================================
loss = lam_pde * loss_pde + lam_bc * loss_bc

# unweighted loss for L-BFGS polish
loss_unweighted = loss_pde + loss_bc
...
def train_adam(n_iters=8000, batch_coll=4000, batch_bc=200):
    ...
    for it in range(n_iters):
        ...
        if it % 500 == 0:
            l_tot, l_pde, l_bc, lr_, lb_ = sess.run(
                [loss, loss_pde, loss_bc, lam_pde, lam_bc],
                feed_dict=feed)
            print("Adam iter {:6d} | loss {:.3e} | pde {:.3e} | bc {:.3e} "
                  "| λ_pde {:.3f} | λ_bc {:.3f} | time {:.2f}s".format(
                  it, l_tot, l_pde, l_bc, lr_, lb_, time.time()-t0))
            t0 = time.time()
...
if __name__ == "__main__":
    print("=" * 65)
    print("  SA-PINN  Bioheat Equation  —  Adam 8000 iters")
    print("=" * 65)
    ...
    np.save("sapinn_bioheat_time.npy", np.array([total_time]))

    print("Training complete. Evaluating solution...")

    # Final SA weights
    lamr_f, lamb_f = sess.run([lam_pde, lam_bc])
    print(f"\n  Final SA weights:  λ_pde={lamr_f:.4f}   λ_bc={lamb_f:.4f}")
    ...
    plt.title("SA-PINN: theta(Fo) at x*")
    plt.legend(); plt.grid(True); plt.tight_layout()
    plt.savefig("sapinn_bioheat_slice.png", dpi=150)
    plt.show()

    plt.figure(figsize=(6, 4))
    plt.pcolormesh(xs, Fos, Theta, shading='auto')
    plt.xlabel("x"); plt.ylabel("Fo")
    plt.title("SA-PINN: theta(x, Fo)")
    plt.colorbar(); plt.tight_layout()
    plt.savefig("sapinn_bioheat_heatmap.png", dpi=150)
    plt.show()

    # ----------------------
    # Save solution for comparison
    # ----------------------
    np.save("sapinn_bioheat.npy", Theta)
    print("Saved: sapinn_bioheat.npy")