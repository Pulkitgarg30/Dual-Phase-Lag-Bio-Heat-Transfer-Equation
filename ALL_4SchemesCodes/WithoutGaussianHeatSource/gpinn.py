...
if __name__ == "__main__":
    ...
    np.save("gpinn_poly_time.npy", np.array([total_time]))
    ...
    plt.title("gPINN POLY: theta(Fo) at x*")
    plt.legend(); plt.grid(True); plt.tight_layout()
    plt.savefig("gpinn_poly_slice.png", dpi=150)
    plt.show()

    plt.figure(figsize=(6, 4))
    plt.pcolormesh(xs, Fos, Theta, shading='auto')
    plt.xlabel("x"); plt.ylabel("Fo")
    plt.title("gPINN POLY: theta(x, Fo)")
    plt.colorbar(); plt.tight_layout()
    plt.savefig("gpinn_poly_heatmap.png", dpi=150)
    plt.show()

    # ----------------------
    # Save solution for comparison
    # ----------------------
    np.save("gpinn_poly.npy", Theta)
    print("Saved: gpinn_poly.npy")