...
if __name__ == "__main__":
    ...
    np.save("pinn_poly_time.npy", np.array([total_time]))
    ...
    plt.xlabel("Fo"); plt.ylabel("theta"); plt.title("PINN POLY: theta(Fo) at x*")
    plt.legend(); plt.grid(True); plt.show()

    # heatmap
    plt.figure(figsize=(6,4))
    plt.pcolormesh(xs, Fos, Theta, shading='auto')
    plt.xlabel("x"); plt.ylabel("Fo"); plt.title("PINN POLY: theta(x,Fo)")
    plt.colorbar(); plt.show()

    # ----------------------
    # Save solution for comparison
    # ----------------------
    np.save("pinn_poly.npy", Theta)
    print("Saved: pinn_poly.npy")