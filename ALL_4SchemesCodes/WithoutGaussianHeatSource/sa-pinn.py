...
if __name__ == "__main__":
    ...
    np.save("sapinn_poly_time.npy", np.array([total_time]))
    ...
    plt.title("SA-PINN POLY: theta(Fo) at x*")
    plt.legend(); plt.grid(True); plt.tight_layout()
    plt.savefig("sapinn_poly_slice.png", dpi=150)
    plt.show()

    plt.figure(figsize=(6, 4))
    plt.pcolormesh(xs, Fos, Theta, shading='auto')
    plt.xlabel("x"); plt.ylabel("Fo")
    plt.title("SA-PINN POLY: theta(x, Fo)")
    plt.colorbar(); plt.tight_layout()
    plt.savefig("sapinn_poly_heatmap.png", dpi=150)
    plt.show()

    # ----------------------
    # Save solution for comparison
    # ----------------------
    np.save("sapinn_poly.npy", Theta)
    print("Saved: sapinn_poly.npy")